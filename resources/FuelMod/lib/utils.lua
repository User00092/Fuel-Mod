...
utils.create_blips = function()
	...
	for _, coords in ipairs(fuel.GAS_STATION_COORDS) do
		...
		HUD.SET_BLIP_AS_SHORT_RANGE(hubBlip, config_handler:get("short_range_blips", true));
		table.insert(utils.created_blips, {
			blip = hubBlip,
			coords_ = coords
		});
...
utils.remove_vehicle = function(v_handle)
	if v_handle == nil then
		return;
	end;
	if utils.vehicle_manager_options[v_handle] then
		utils.remove_menu_ref(utils.vehicle_manager_options[v_handle].root);
	end;
